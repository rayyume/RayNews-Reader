...
RayNews 是一个以 Telegram 公开频道为数据入口的自托管新闻聚合阅读器。它可以增量抓取频道消息、提取 Telegraph ~~和微信公众号原抓取方式因被微信封堵不再支持~~等文章全文，并提供 AI 摘要、翻译、每日总结、订阅源管理、收藏和图片持久缓存。
...
- 从 Telegram 公开频道每 15 分钟增量抓取新闻，也支持手动刷新
  首次运行仅导入北京时间当天的消息；后续增量抓取保留跨日积压，中断恢复后可能批量补抓，每轮最多 200 页。补抓文章按入库时间进入日报窗口，详见[变更说明](release_note_unreleased.md)。
...
- 来源分类与媒体身份相互独立；AI 仅辅助分类，高置信度建议自动应用，低置信度建议由管理员确认
...
- 每日摘要由服务端在北京时间每天 21:00 用管理员配置的服务端 API 统一生成一次，不支持手动触发；
  ...
  服务端先从单篇摘要提取事件线索，按具体事件合并、根据事件影响和独立来源数排序，最多收录 60 条；
